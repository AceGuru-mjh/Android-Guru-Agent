...
/**
 * # Code Thinking Guide Sheet — Coding 屏思考档位指南（v1.2 七档思考系统）
 *
 * ## 这个组件解决什么问题
 *
 ...
 *   输出预算），数据**直读 [ThinkingProfile.forLevel] 静态表**——UI 与引擎
 *   画像永远同源，杜绝文案漂移（core 改画像数值，这里自动跟读）；
 * - **每档一张卡片**：档位名徽标 + [ThinkingLevel.description]（枚举一句话）
 *   + [ThinkingProfile.uiDescriptionZh]（画像执行策略摘要）+ 「编码视角」
 ...
 * - 「编码视角」8 段文案手写在本文件（对齐 [ThinkingProfile.uiDescriptionZh]
    ...
    ModalBottomSheet(
        ...
        Column(
            ...
            LazyColumn(
                ...
                items(ThinkingLevel.entries, key = { it.name }) { level ->
                    TierCard(
                        level = level,
 ...
 *
 * 数据全部取自 [ThinkingProfile.forLevel]——budget / effort / 迭代倍率 /
...
private fun TierCard(
    ...
    val profile = remember(level) { ThinkingProfile.forLevel(level) }
    ...
    Card(
        ...
        Column(
            ...
            // ── 枚举一句话（ThinkingLevel.description）──
            Text(
...
@Composable
private fun TierBadge(level: ThinkingLevel) {
    val color = when (level) {
        ThinkingLevel.APEXCODE -> MaterialTheme.colorScheme.error
        ThinkingLevel.ULTRACODE -> MaterialTheme.colorScheme.tertiary
        ThinkingLevel.MAXIMUM, ThinkingLevel.DEEP -> MaterialTheme.colorScheme.primary
 ...
 *
 * 手写中文（与 [ThinkingProfile.uiDescriptionZh] 的 core 侧中文风格对齐），
 ...
 */
private fun codingViewText(level: ThinkingLevel): String = when (level) {
    ThinkingLevel.NONE ->
