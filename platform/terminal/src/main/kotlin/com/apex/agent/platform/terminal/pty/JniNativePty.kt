 ...
 *   read(sessionId, buffer, maxBytes): Int                      →  nativeRead(sessionId, maxBytes, stripAnsi=false): String  (copy into buffer)
 ...
 * Bytes are copied into the caller's buffer; if the string is longer than maxBytes, it's truncated.
 */
class JniNativePty(
    ...
    override fun nativeRead(sessionId: Int, buffer: ByteArray, maxBytes: Int): Int {
        // Existing JNI returns a String (UTF-8). We pass stripAnsi=false — VT100Emulator parses ANSI.
        ...
        if (result.isEmpty()) {
            ...
            // Use nativeHasData to disambiguate.
